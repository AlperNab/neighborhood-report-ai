...
**Domain:** `Real Estate / Location Intelligence` · **Suite:** `General Automation Suite` · **Accent:** `#93c5fd`
...
**Neighborhood Report Ai** is a standalone, browser-based AI workflow app for **Real Estate / Location Intelligence**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `address`<br>Address | text | Yes | — | Affects input: Address. |
...
| `city_country`<br>city/country | select | No | United States | Affects customization: city/country. |
| `buyer_renter_type`<br>buyer/renter type | text | No | — | Affects customization: buyer/renter type. |
| `family_investor_focus`<br>family/investor focus | text | No | — | Affects customization: family/investor focus. |
| `commute_address`<br>commute address | text | No | — | Affects customization: commute address. |
| `budget`<br>budget | text | No | — | Affects customization: budget. |
| `priority_weights`<br>priority weights | text | No | — | Affects customization: priority weights. |
...
- **city/country**: United States, United Kingdom, Egypt, UAE, Saudi Arabia, Germany, Custom
...
uvicorn app.main:app --host 127.0.0.1 --port 9140
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9140`. |
...
- Export download
- Job history
- Maps/geocoding/school/crime/flood/transport APIs required for live facts

### Real Connector Requirements

- maps/geocoding provider
- property/comparable-data source
...
```bash
curl http://127.0.0.1:9140/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9140/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9140 uvicorn app.main:app --host 0.0.0.0 --port 9140
```
...
- `plugins/neighborhood-report-ai.json` for plugin metadata.
